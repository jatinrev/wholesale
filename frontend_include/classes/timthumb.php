...
//These are now disabled by default because the file sizes of PNGs (and GIFs) are much smaller then we used to generate. 
//They only work for PNGs. GIFs and JPEGs are not affected.
if(! defined('OPTIPNG_ENABLED') ) 		define ('OPTIPNG_ENABLED', false);  
if(! defined('OPTIPNG_PATH') ) 			define ('OPTIPNG_PATH', '/usr/bin/optipng'); //This will run first because it gives better compression then pngcrush. 
...
*/
if(! defined('WEBSHOT_ENABLED') ) 	define ('WEBSHOT_ENABLED', false);			//Beta feature. Adding webshot=1 to your query string will cause the script to return a browser screenshot rather then try to fetch an image.
...
class timthumb {
	...
	protected $filePrependSecurityBlock = "<?php die('Execution denied!'); //"; //Designed to have three letter mime type, space, question mark and greater then symbol appended. 6 bytes total.
			...
			if($this->isURL){
				...
				if(filesize($this->cachefile) < 1){
					...
					if(time() - @filemtime($this->cachefile) > WAIT_BETWEEN_FETCH_ERRORS){
						$this->debug(3, "File is older then " . WAIT_BETWEEN_FETCH_ERRORS . " seconds. Deleting and returning false so app can try and load file.");
	...
	protected function serveInternalImage(){
		...
		if($fileSize > MAX_FILE_SIZE){
			$this->error("The file you specified is greater then the maximum allowed file size.");
			return false;
		}
		if($fileSize <= 0){
	...
	protected function cleanCache(){
		...
		if(@filemtime($lastCleanFile) < (time() - FILE_CACHE_TIME_BETWEEN_CLEANS) ){ //Cache was last cleaned more then 1 day ago
			$this->debug(1, "Cache was last cleaned more then " . FILE_CACHE_TIME_BETWEEN_CLEANS . " seconds ago. Cleaning now.");
			...
			if ($files) {
				...
				foreach($files as $file){
					if(@filemtime($file) < $timeAgo){
						$this->debug(3, "Deleting cache file $file older then max age: " . FILE_CACHE_MAX_FILE_AGE . " seconds");
						@unlink($file);
					}
				}
			}
			...
			$this->debug(3, "Cache was cleaned less then " . FILE_CACHE_TIME_BETWEEN_CLEANS . " seconds ago so no cleaning needed.");
		}
		return false;
	}
